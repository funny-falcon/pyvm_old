#
# pyvm NOTE: atm pyvm does not have a builtin XML parser.
# expat uses recursion (!) and callbacks (!!) to parse XML
# and except from efficiency, this does not play well with
# stackless. An efficient XML parser could be written in
# less than 1000 lines but we got to know about wtf8 first.
#
# For now xmllib is the only backend.
#
#
			...
			if rawdata[i] == '<':
				...
				if res is not None:
					...
					if name in self.entitydefs:
						name = self.entitydefs [name]
						chre = charref (name)
						if chre:
							# sx: very inefficient to replace charref!!
							if not self.stack:
								self.syntax_error('data not in content')
							self.handle_charref(res.group('char')[:-1])
							self.lineno=self.lineno+res.group(0).count('\n')
							continue
						self.rawdata = rawdata = rawdata[:res.start(0)] + name + rawdata[i:]
						n = len(rawdata)
						i = res.start(0)
					else:
