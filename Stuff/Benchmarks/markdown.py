...
#
# pyvm: for the official version check out <http://webpy.org>
#
...
class _Markdown:
    ...
    def parse(self, text):
        ...
        text = self._RunBlockGamut(text, 1)
    ...
    def _RunBlockGamut(self, text, xxx=0):
...
#
# This is not a very fair test for pyvm because it uses re.sub
# heavily and atm this is not optimized in pyvm (see re.py).
# Still, it performs pretty well.
#
# this is a great test for regular expressions.
#
from mdtests import D as TESTS
from time import time

for i, j in TESTS.items():
	if 'SEM' not in sys.argv:
		t0=time()
		for n in xrange ((300000/len (j))/10):
			Markdown (j)
		t0 = time ()-t0
		print i, t0
		continue
