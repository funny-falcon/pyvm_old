...
'GROUP':'real-bench',
'CMPOUT':1,
'DATA':'DATA/gtkwidget.xml',
'ARGS':"DATA/xx.xml SEM",
'BARGS':"DATA/gtkwidget.xml"
}
'''

## changes for pyvm. some optimizations -- sxanth
...
illegal = re.compile('[^\t\r\n -\176\240-\377]').search # illegal chars in content
interesting = re.compile('[]&<]').search

amp = re.compile('&').search
entityref = re.compile('&(?P<name>' + _Name + ')[^-a-zA-Z0-9._:]').match
charref = re.compile('&#(?P<char>[0-9]+[^0-9]|x[0-9a-fA-F]+[^0-9a-fA-F])').match
space = re.compile(_S + '$').match
newline = re.compile('\n')
...
starttagmatch = re.compile('<(?P<tagname>'+_Name+')'
                      ...
                      starttagend.pattern).match
attrfind = attrfind.match
endtagopen = re.compile('</').match

DOJIT=1
try:
	tagfind = re.compile(_Name, dojit=DOJIT).match
	endbracketfind = re.compile('(?:[^>\'"]|'+_QStr+')*>', dojit=DOJIT).match
	starttagopen = re.compile('<' + _Name, dojit=DOJIT).match
	ref = re.compile('&(' + _Name + '|#[0-9]+|#x[0-9a-fA-F]+)[^-a-zA-Z0-9._:]', dojit=DOJIT).match
	endbracket = re.compile(_opS + '>', dojit=DOJIT).match
except:
	tagfind = re.compile(_Name).match
	endbracketfind = re.compile('(?:[^>\'"]|'+_QStr+')*>').match
	starttagopen = re.compile('<' + _Name).match
	ref = re.compile('&(' + _Name + '|#[0-9]+|#x[0-9a-fA-F]+)[^-a-zA-Z0-9._:]').match
	endbracket = re.compile(_opS + '>').match

cdataopen = re.compile(r'<!\[CDATA\[').match
...
xmlns = re.compile('xmlns(?::(?P<ncname>'+_NCName+'))?$').match
...
class XMLParser:
	...
	def goahead(self, end):
		...
		while i < n:
			...
			if i < j:
				data = rawdata[i:j]
				if self.__at_start and space(data) is None:
					self.syntax_error('illegal data at start of file')
				self.__at_start = 0
				if not self.stack and space(data) is None:
					self.syntax_error('data not in content')
				if not self.__accept_utf8 and illegal(data):
			...
			if rawdata[i] == '<':
				...
				if commentopen(rawdata, i):
					...
					i = k
					continue
				if cdataopen(rawdata, i):
					k = self.parse_cdata(i)
					if k < 0: break
					self.lineno = self.lineno + rawdata[i:k].count('\n')
					i = k
					continue
				res = xmldecl(rawdata, i)
				if res:
				...
				if res is not None:
					...
					if name in self.entitydefs:
						cref = charref (self.entitydefs [name])
						if cref:
							if not self.stack:
								self.syntax_error('data not in content')
							self.handle_charref(cref.group('char')[:-1])
							self.lineno = self.lineno+res.group(0).count('\n')
							continue
	...
	def parse_comment(self, i):
		...
		if not self.__accept_utf8 and \
        	   illegal(rawdata, i+4, res.start(0)):
	...
	def parse_doctype(self, res):
		...
		if rawdata[k] == '[':
			...
			while k < n:
				...
				k = k+1
		res = endbracketfind(rawdata, k)
		if res is None:
			return -1
		if endbracket(rawdata, k) is None:
	...
	def parse_cdata(self, i):
		...
		if res is None:
			return -1
		if not self.__accept_utf8 and \
           illegal(rawdata, i+9, res.start(0)):
	...
	def parse_proc(self, i):
		...
		if end is None:
			return -1
		j = end.start(0)
		if not self.__accept_utf8 and illegal(rawdata, i+2, j):
			self.syntax_error('illegal character in processing instruction')
		res = tagfind(rawdata, i+2)
		if res is None:
	...
	def parse_starttag(self, i):
		...
		end = endbracketfind(rawdata, i+1)
		if end is None:
			return -1
		tag = starttagmatch(rawdata, i)
...
def test(args = None, cooked = False):
	import sys
	klass = TestXMLParser
	f = open (sys.argv [1])
	data = f.read()
	...
	if cooked:
		...
		# this is mostly a test of the regular expression engines
		x.feed (data)
	x.close()
