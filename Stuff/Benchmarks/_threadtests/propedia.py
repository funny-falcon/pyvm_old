...
class MultiThread:
    def __init__( self, function, argsVector, maxThreads=7 ):
...
def recite_n_times_table( n ):
    for i in range( 1, 5 ):
        print "%d * %d = %d" % (n, i, n * i)
        time.sleep( 0.00005 + random.random() )

if __name__=="__main__":
   mt = MultiThread( recite_n_times_table, range( 1, 5 ) )
   mt.start()
   mt.join()
