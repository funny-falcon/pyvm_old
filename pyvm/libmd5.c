...
MD5Update(MD5_CTX *context, const unsigned char *input, unsigned int inputLen)
{
    ...
    if (inputLen >= partLen) {
        __builtin_memcpy((POINTER)&context->buffer[index], (POINTER)input, partLen);
    ...
    __builtin_memcpy((POINTER)&context->buffer[index],
...
MD5Final(unsigned char digest[16], MD5_CTX *context)
{
    ...
    __builtin_memset((POINTER)context, 0, sizeof (*context));
}
...
MD5Transform(UINT4 state[4], const unsigned char block[64])
{
    ...
    /* Zeroize sensitive information. */
//    __builtin_memset((POINTER)x, 0, sizeof (x));
}
