/*
	...
	This is standard dl-malloc but:
	- Since we are using the seg-allocator for small/fast things
	we may be able to tune/inline/constify some parameters
	- We remove some code (mallinfo) to reduce the size of the library
	- ifdef'd out:
		valloc, pvalloc, independent_*alloc, aLLOc, cfree,
		malloc_trim, malloc_usable_size
	  we don't need any of these in pyvm. Perhaps we could use
	  independents* later...
...
#else
#define SMALLBIN_WIDTH      6
#define MIN_LARGE_SIZE     64
#endif
